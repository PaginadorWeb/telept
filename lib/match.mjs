...
export function findBestMatch(phones, product) {
  ...
  const tokensOf = (phone) => tokenize(phone.model);

  for (const phone of phones) {
    if (brand && phone.brand?.toLowerCase() !== brand.toLowerCase()) continue;
    const modelTokens = tokensOf(phone);
    ...
    if (s === 0) continue;
    const bestTokens = best ? tokensOf(best) : [];
    if (modelTokens.length > bestTokens.length || (modelTokens.length === bestTokens.length && norm(phone.model).length > norm(best.model).length)) {
      best = phone;
    }
  }
